...
ME.full<-function(x,y,w,model,Hessian,
  ...
  return(list(Estimates.vec=estimates.vec,
              ...
              Hess.mat=Hess.mat,optim.result=optimresult))
                                        ...
                                        #This algorithm is used to FML, and OML using CCA.
...
partioningdata<-function(x,y,w,no){
  
  
  woo<-as(w[1:no,1:no], "CsparseMatrix")
  xo<-x[1:no,]
  yo<-y[1:no]
  
  return(list(xo,yo,woo))
  
} # This function extracts only the observed data (first `no` rows)
                                        # and is used for OML estimation.
